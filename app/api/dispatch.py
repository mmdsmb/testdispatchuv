...
async def start_dispatch(
    ...
    """Endpoint pour lancer l'exécution du script test_dispatch.py"""
    ...
    background_tasks.add_task(run_dispatch_script, task_id, date_begin, date_end, milp_timeout)
...
async def get_results_supabase(
    ...
    try:
        ...
        # Convertir le tuple en dictionnaire si nécessaire
        if isinstance(task_data, tuple):
            columns = ['id', 'status', 'start_time', 'elapsed_time_min', 'result_file', 'output', 'error', 'results']
            task_data = dict(zip(columns, task_data))
        
        # Extraire les informations importantes de la sortie du solveur
        output = task_data.get("output", "")
        solver_info = {}
        
        if output:
            # Extraire le coût total si présent
            import re
            cost_match = re.search(r"Solution heuristique obtenue avec coût total approximatif : ([\d.]+)", output)
            if cost_match:
                solver_info["total_cost"] = float(cost_match.group(1))
            
            # Extraire les informations du solveur
            solver_info["iterations"] = re.search(r"Total iterations:\s*(\d+)", output)
            solver_info["nodes"] = re.search(r"Enumerated nodes:\s*(\d+)", output)
            solver_info["time_cpu"] = re.search(r"Time \(CPU seconds\):\s*([\d.]+)", output)
            solver_info["time_wall"] = re.search(r"Time \(Wallclock seconds\):\s*([\d.]+)", output)
            
            # Nettoyer les valeurs extraites
            for key in solver_info:
                if solver_info[key]:
                    solver_info[key] = float(solver_info[key].group(1))
...
async def run_dispatch_script(task_id: str, date_begin: Optional[str] = None, date_end: Optional[str] = None, milp_timeout: int = 300):
    """Exécute le script test_dispatch.py en arrière-plan"""
    try:
        # Mise à jour du statut
        tasks[task_id]["status"] = "running"
        
        # Commande pour exécuter le script test_dispatch.py
        cmd = ["python", "test_dispatch.py"]

        # Ajouter les paramètres si fournis
        if date_begin:
            cmd.extend(["--date_begin", date_begin])
        if date_end:
            cmd.extend(["--date_end", date_end])
        cmd.extend(["--milp_timeout", str(milp_timeout)])
        ...
        # Mettre à jour le statut en fonction de la sortie
        ...
        await save_task_to_supabase(
            task_id, tasks[task_id]["status"], tasks[task_id]["start_time"], 
...
async def save_task_to_supabase(task_id, status, start_time, elapsed_time, result_file, output, error, results=None):
    ...
    try:
        ...
        # Vérifier si la table `tasks` existe, sinon la créer
        check_table_query = """
        ...
        await ds.execute_transaction([
            (check_table_query, None),  # Création de table
            (insert_query, (task_id, status, start_time_iso, elapsed_time/60, result_file, output, error, results))  # Insertion
        ])
