...
class RouteAnalyzer:
        ...
        for i, label in enumerate(clustering.labels_):
            if label >= 0:
                ...
                clusters[label].append({
                    'index': original_idx,
                    'origin': window_df.loc[original_idx, 'lieu_prise_en_charge'],
                    'destination': window_df.loc[original_idx, 'destination'],
    ...
    def analyze_routes(self, df, datetime_column, output_file='trajets_similaires.csv'):
        ...
        required_columns = ['points_passage_coords', 'lieu_prise_en_charge', 'destination']
